...
def splice_rna(dna, intron_list):   #splits dna into exons, removing the introns. then translates in into rna, and then into a protein string
    for intron in intron_list:
        dna.replace(intron,"#")
    exons=dna.split("#")
    protein=[]
    for exon in exons:
        exon_rna= dna2rna(exon)  #do I need to invert it also? It wasn't clear and I can't test it
        protein.append(rna2codon(exon_rna))
    protein_str= protein.join()
    return protein_str
print (reverse_complement(dna))


#returns the number of possible source rna strings that could make a protein mod 1,000,000
...
#returns the average number of offspring displaying the dominant trait given a list of integers representing types of pairs. 
#AA-AA
#AA-Aa
#AA-aa
#Aa-Aa
